...
*   `this.target`：等于 Webpack 配置中的 Target，详情见 [2-7其它配置项-Target](../chapter2/quarter2-7.html#Target)。
...
在 [2-7其它配置项](../chapter2/quarter2-7.html#ResolveLoader) 中曾介绍过 ResolveLoader 用于配置 Webpack 如何寻找 Loader。 默认情况下只会去 `node_modules` 目录下寻找，为了让 Webpack 加载放在本地项目中的 Loader 需要修改 `resolveLoader.modules`。
